...
# Endpoint to generate descriptions
...
async def generate_descriptions(
    ...
    try:
        ...
        project_data = pd.read_csv(io.BytesIO(contents))  # Convert to DataFrame

        # Convert team_members string to a list of names
        ...
        if team_member_list:
            project_data = project_data[project_data["Participant"].isin(team_member_list)]
        ...
        for focus, dimensions in focus_to_dimensions.items():
            ...
            if focus == "Product-Market-Fit":
                ...
                prompt = (
                    ...
                    f"Here are the aggregate measures of the team scores per dimension:\n"
                    f"{'\n'.join(aggregate_texts)}\n\n"
                    f"Provide an analysis that identifies where team members fall within each of the three phases of the business performance dialogue based on their score profiles: 1. Changing with people (High Exploration combined with high Attachment), 2. Operational Core (Mid-range exploration combined with mid-range attachment), 3. Structured delivery (low exploration combined with low attachment). Highlights how these contributions shape the team's performance and collaboration in each phase (e.g., driving change, converting ideas into projects, ensuring execution). Points out any gaps or underrepresented contributions in specific phases, particularly where contributions from either Attachment or Exploration might be lacking. Suggests strategies to address these gaps, ensuring smoother transitions across phases. Ideally, a team has approximately equal ratios of team members in each of the three phases."
                )
